...
def generate_distributor_file(filtered_df, date):
    for key, value in filtered_df.groupby('Distributor'):
        ...
        for territory, territory_df in value.groupby('Territory'):
            territory_pivot = pd.pivot_table(territory_df, values='Distributor Stock', index=['Statistics Model'], columns=['Store/Warehouse'], aggfunc=np.sum, fill_value=0)
            territory_pivot = territory_pivot.replace(0, np.nan)
            territory_pivot.to_excel(writer, sheet_name=territory)
        writer.save()
...
def main():
    ...
    generate_dos_file(filtered_df, today)
    generate_distributor_file(filtered_df, today)
